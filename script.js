// ==========================================
// 1. UI/UX ANIMATIONS (LENIS & GSAP CURSOR)
// ==========================================
...
window.addEventListener("load", () => {
  gsap.from(".reveal-anim", { y: 50, opacity: 0, duration: 1.5, stagger: 0.2, ease: "expo.out" });
});
...
document.querySelectorAll('.btn-toggle').forEach(btn => {
  btn.addEventListener('click', () => {
    document.querySelectorAll('.btn-toggle').forEach(b => b.classList.remove('active-toggle'));
    ...
    if(val === 'y') {
      feeOptionsWrapper.classList.remove('hidden');
      gsap.fromTo(feeOptionsWrapper, {opacity: 0, y: -10}, {opacity: 1, y: 0, duration: 0.4});
    } else {
      feeOptionsWrapper.classList.add('hidden');
    }
  });
});
  ...
  reader.onload = (event) => {
    ...
    img.onload = () => {
      ...
      if (code) {
        ...
        alert("Gagal membaca QRIS. Pastikan gambar jelas.");
      }
    };
...
function displayMerchantInfo(qrisCode) {
  ...
  if (nameMatch) {
    ...
    merchantInfoElement.innerHTML = `<p class="text-[10px] text-blue-500 uppercase font-bold tracking-widest mb-1">Merchant Detected</p><p class="text-white font-black text-lg">${name}</p>`;
    merchantInfoElement.classList.remove('hidden');
    gsap.from(merchantInfoElement, {opacity: 0, x: -20, duration: 0.5});
  }
}
...
qrisForm.addEventListener('submit', (e) => {
  e.preventDefault();
  if (!qrisStaticCode) return alert('Silakan upload gambar QRIS statis Anda terlebih dahulu, Jenderal.');
  ...
  new QRCode(qrcodeElement, { text: finalQR, width: 200, height: 200, colorDark : "#000000", colorLight : "#ffffff" });
  ...
  setTimeout(() => {
    ...
    setTimeout(() => { 
      ...
      gsap.from(downloadBtn, {opacity: 0, y: 10, duration: 0.5});
    }, 800);
  }, 100);
  
  // Auto scroll to result
  setTimeout(() => {
    lenis.scrollTo(resultArea, { offset: -50 });
  }, 300);
});
...
downloadBtn.addEventListener('click', () => {
  ...
  link.href = img.src; 
  link.download = `QRIS-Dynamic-Azmi-${Date.now()}.png`;
